...
setUpInformationTab <- function(new.dataset=TRUE){
...
#      executeExplorePlotBuilder()
  ...
  else if (theWidget("explore_information_radiobutton")$getActive()){
	executeExploreInformation(ndataset)
  }
}
...
if (FALSE)
{
  ...
  digit <- 1
  len <- 1
  ds <- read.csv(system.file("csv/audit.csv", package="rattleM"))
...
on_explore_mutualinformation_radiobutton_toggled <- function(button)
{
  if (button$getActive()) crv$EXPLORE$setCurrentPage(crv$EXPLORE.INFORMATION.TAB)
  setStatusBar()
}
#-----------------------------------------------------------------------
# DISTRIBUTIONS
