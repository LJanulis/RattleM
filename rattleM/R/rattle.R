...
if(getRversion() >= "2.15.1")
  utils::globalVariables(c("rattleM.entered.dataset",
                           "gladeXMLNew",
...
#    paste(...)
#  else
    gettext(paste(...), domain="R-rattleM")
}
...
# 091223 Rtxt does not work until the rattleM GUI has started, perhaps?
...
rattleM <- function(csvname=NULL, dataset=NULL, useGtkBuilder=TRUE)
{
  ...
  if (! RGtk2::gtkInit())
    stop("Can't load RGtk2.\n",
         ...
         "  Running rattleM() locally on a desktop is the default.")
...
#              sprintf("%s\n%s\n", Sys.time(), paste(rep("^", 72), collapse=""))))
  ...
  # Try firstly to load the glade file from the installed rattleM
  # package, if it exists. Otherwise, look locally.

  if (crv$useGtkBuilder)
  {
    crv$rattleGUI <- RGtk2::gtkBuilderNew()
    crv$rattleGUI$setTranslationDomain("R-rattleM")
  }
  
  result <- try(etc <- file.path(path.package(package="rattleM")[1], "etc"),
                silent=TRUE)
  if (inherits(result, "try-error"))
    if (crv$useGtkBuilder)
      crv$rattleGUI$addFromFile(crv$rattleUI)
    else
      crv$rattleGUI <- gladeXMLNew("rattleM.glade",
                                root="rattle_window", domain="R-rattleM")
  else
    if (crv$useGtkBuilder)
      crv$rattleGUI$addFromFile(file.path(etc, crv$rattleUI))
    else
      crv$rattleGUI <- gladeXMLNew(file.path(etc,"rattleM.glade"),
                                root="rattle_window", domain="R-rattleM")
...
#        format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
  ...
  if (!missing(csvname) && !missing(dataset))
  {
    errorDialog(Rtxt("Both a csvname= and a dataset= were specified in",
                     "the call to rattleM(). At most, only one is allowed.",
  ...
  if (is.null(csvname) && is.null(dataset))
  {
    ...
    if ((.rattleM.data <- Sys.getenv("RATTLE_DATA")) != "")
      csvname <- .rattleM.data
  }
  ...
  # location for finding rattleM.glade. Assumes the call to source is
  ...
  crv$PREDICT <- c(crv$RPART, crv$ADA, crv$RF, crv$KSVM, crv$GLM,
                     crv$NNET, crv$SURVIVAL, crv$CRULES, crv$J48, crv$C50)
...
#    crv$NOTEBOOK.MODEL.NAME <- Rtxt("Predictive")
  ...
  crv$EXPLORE.INFORMATION.TAB <- getNotebookPage(crv$EXPLORE, "information")
...
##  while (gtkEventsPending()) gtkMainIteration() # Make sure window is displayed
  ...
  # Now deal with any arguments to rattleM.
...
configureGUI <- function()
{
  ...
  rattleM.menu <- theWidget("rattle_menu")
  rattleM.menu$SetRightJustified(TRUE)
  rattleM.menu$getChild()$setMarkup(id.string)
  ...
  crv$icon <- system.file("etc/Rlogo.png", package="rattleM")
  ...
  connectr.logo <- system.file("etc/ConnectRlogo.png", package="rattleM")
  if (packageVersion("RGtk2") == "2.20.31")
...
#LOG_LICENSE
                       ...
                       Rtxt("Rattle was created and implemented by Graham Williams",
                            "with contributions as acknowledged in 'library(help=rattleM)'.")),
                tvsep=FALSE)
}
...
##   theWidget("odbc_believeNRows_checkbutton")$setActive(FALSE)
  ...
  if (new.dataset)
  {
	...
	#theWidget("crulestreeview")$getModel()$clear()
	#theWidget("dectrees_textview")$getModel()$clear()
...
reportTimeTaken <- function(tv, time.taken, model, msg)
{
  ...
  if (missing(model) && missing(msg) || (!missing(model) && !missing(msg)))
    stop(Rtxt("rattleM: reportTimeTaken:",
  ...
  if (!exists("RStudioGD"))
  {
        ...
        packageIsAvailable("cairoDevice", Rtxt("display plots")))
    {
      if (crv$useGtkBuilder)
      {
        ...
        plotGUI$setTranslationDomain("R-rattleM")
      }
      ...
      if (inherits(result, "try-error"))
        if (crv$useGtkBuilder)
          ...
          plotGUI <- gladeXMLNew("rattleM.glade", root="plot_window", domain="R-rattleM")
      else
        if (crv$useGtkBuilder)
          plotGUI$addFromFile(file.path(etc, crv$rattleUI))
        else
          plotGUI <- gladeXMLNew(file.path(etc,"rattleM.glade"),
                                 root="plot_window", domain="R-rattleM")
      if (crv$useGtkBuilder)
      {
...
closeRattle <- function(ask=FALSE)
{
  ...
  # rattleM script on GNU/Linux using the littler package which allows
  # one to use R as a scripting language. But rattleM dispatches
  ...
  # 080906 Deal with R not finishing up when rattleM is called from
...
on_about_menu_activate <-  function(action, window)
{
  ...
  if (crv$useGtkBuilder)
  {
    ...
    about$setTranslationDomain("R-rattleM")
  }
  ...
  if (inherits(result, "try-error"))
    ...
    about <- gladeXMLNew("rattleM.glade", root="aboutdialog", domain="R-rattleM")
  else
    if (crv$useGtkBuilder)
      about$addFromFile(file.path(etc, crv$rattleUI))
    else
      about <- gladeXMLNew(file.path(etc, "rattleM.glade"),
                           root="aboutdialog", domain="R-rattleM")
