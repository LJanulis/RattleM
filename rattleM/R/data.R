...
executeDataCSV <- function(filename=NULL)
{
                ...
                theWidget("data_xdf_checkbutton")$getActive()))
  {
    ...
    if (system.file("csv", "weather.csv", package="rattleM") ==
          display_filename)
...
viewData <- function()
{
  ...
  else
  {
    result <- try(etc <- file.path(path.package(package="rattleM")[1], "etc"),
                  silent=TRUE)
    if (inherits(result, "try-error"))
      crs$viewdataGUI <- gladeXMLNew("rattleM.glade", root="viewdata_window")
    else
      crs$viewdataGUI <- gladeXMLNew(file.path(etc,"rattleM.glade"),
...
getNumericVariables <- function(type="string")
{
  ...
  if (length(nums) > 0)
  {
	...
	if(type == "names"){
		return(names(crs$dataset)[indicies])
	}
  }
  else
    indicies <- NULL
