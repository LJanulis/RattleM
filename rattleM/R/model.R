...
setTreeOptions <- function(mtype)
{
  ...
  theWidget("model_tree_include_missing_checkbutton")$setSensitive(mtype=="rpart")
}
...
commonName <- function(mtype)
{
  name.map <- data.frame(
	...
	crules=Rtxt("RIPPER Algorithm Classification Rules"),
	j48=Rtxt("RWeka implementation of C4.5 decision tree algorithm J48"),
	c50=Rtxt("C5.0 decision tree classifier"))
...
rattleM.print.summary.multinom <- function (x, digits = x$digits, ...) 
{
