                       ...
                       include.libpath=FALSE)
{
  ...
  riv <- iv["rattleM", "Version"]
  if (have.av) rav <- av["rattleM", "Version"]
  ...
  up <- if (have.av && compareVersion(rav, riv) == 1) "rattleM" else NULL
  ...
  if (all.dependencies)
  {
    ...
    if (have.av)
      deps <- pkg.deps("rattleM", NULL, av)
    else
      deps <- pkg.deps("rattleM", NULL, iv)
  }    
  else
    deps <- union(deps2vec(iv["rattleM", "Depends"]), deps2vec(iv["rattleM", "Suggests"]))

  for (p in sort(setdiff(deps, 'rattleM')))
  {
  ...
  if (! is.null(up))
  {
    ...
    if (isWindows() && "rattleM" %in% up)
      cat("Detach rattleM (and other attached packages) before updating:\n\n ",
          '> detach("rattleM")\n\n')
