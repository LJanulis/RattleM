...
#  crv <<- new.env() # 121212 Moved to top of file so not global
  ...
  # file. 130402 Revert to using rattle_macosx.ui - rattleM.ui does not
  # yet work?
  
  crv$rattleUI <- "rattleM.ui"
  ...
  crv$library.command <- "library(rattleM)"
...
#    crv$load.tooltips <- FALSE # Not working in general on Linux
  ...
  crv$project.extensions <- c("rattleM", "rstat") # Extensions for projects  
  ...
  # 091221 The Rtxt does not seem to work from the rattleM.R file, so
  # do it here again.
  ...
  msg <- paste(Rtxt("Rattle: A free graphical interface",
               ...
               COPYRIGHT, "\n",
#LICENSE
               Rtxt("Type 'rattleM()' to shake, rattle, and roll your data."),
               "\n",
               sep="")
    
  if ("rattleM" %in% getOption("defaultPackages"))
    rattleM()
  else
