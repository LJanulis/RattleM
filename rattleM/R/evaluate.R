...
#  						theWidget("evaluate_crules_checkbutton")$getActive() || 		
...
#                      && ! multinomialTarget()))
  ...
  default.to.class <- (theWidget("evaluate_kmeans_checkbutton")$getActive() ||
                       ...
                       theWidget("evaluate_ksvm_checkbutton")$getActive() ||
					   theWidget("evaluate_crules_checkbutton")$getActive() ||
					   theWidget("evaluate_j48_checkbutton")$getActive() ||
					   theWidget("evaluate_c50_checkbutton")$getActive() ||
...
resetEvaluateTab <- function()
{
  ...
  theWidget("evaluate_crules_checkbutton")$setActive(FALSE)
  theWidget("evaluate_crules_checkbutton")$setSensitive(FALSE)
  
  theWidget("evaluate_j48_checkbutton")$setActive(FALSE)
  theWidget("evaluate_j48_checkbutton")$setSensitive(FALSE)
  
  theWidget("evaluate_c50_checkbutton")$setActive(FALSE)
  theWidget("evaluate_c50_checkbutton")$setSensitive(FALSE)
}
...
#      ## gsub(")$", ', type="raw")', predcmd[[crv$NNET]])
  ...
  evRes <- evaluateIfCrulesSelected(mtypes, testset0, testset, predcmd, respcmd, probcmd)
  testset <- evRes$testset
  predcmd <- evRes$predcmd
  respcmd <- evRes$respcmd
  probcmd <- evRes$probcmd
  
  evRes <- evaluateIfJ48Selected(mtypes, testset0, testset, predcmd, respcmd, probcmd)
  testset <- evRes$testset
  predcmd <- evRes$predcmd
  respcmd <- evRes$respcmd
  probcmd <- evRes$probcmd
  
  evRes <- evaluateIfC50Selected(mtypes, testset0, testset, predcmd, respcmd, probcmd)
  testset <- evRes$testset
  predcmd <- evRes$predcmd
  respcmd <- evRes$respcmd
  probcmd <- evRes$probcmd
...
executeEvaluateConfusion <- function(respcmd, testset, testname)
{
  ...
  for (mtype in getEvaluateModels())
  {
    ...
    confuse.cmd <-
      sprintf("rattleM::errorMatrix(%s$%s, crs$pr, count=TRUE)", ts, crs$target)

    percentage.cmd <-
      sprintf('(per <- rattleM::errorMatrix(%s$%s, crs$pr))', ts, crs$target)
...
#                      "will be scored."))
      dsname <- "rattleM.entered.dataset"
      if (exists(dsname))
        rattleM.edit.obj <- RGtk2Extras::dfedit(rattleM.entered.dataset, size=c(800, 400))
      else
        rattleM.edit.obj <- RGtk2Extras::dfedit(crs$dataset[nrow(crs$dataset),
      ...
      RGtk2::gSignalConnect(rattleM.edit.obj, "unrealize", data=rattleM.edit.obj,
                     function(obj, data)
                     {
